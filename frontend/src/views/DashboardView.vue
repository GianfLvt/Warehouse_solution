<template>
  <div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 mb-6">
      ...
      <div class="card p-4 sm:p-5">
        <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Sotto scorta</p>
      ...
      <div class="card p-4 sm:p-5">
        ...
        <p class="text-2xl sm:text-3xl font-bold text-indigo-600 mt-1">{{ stats.total_products }}</p>
      </div>
      <div class="card p-4 sm:p-5">
        <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Totale clienti</p>
        <p class="text-2xl sm:text-3xl font-bold text-purple-600 mt-1">{{ stats.total_customers }}</p>
      </div>
    </div>
...
const stats = ref({
  pending_orders: 0, daily_shipments: 0, low_stock_products: 0,
  monthly_revenue: 0, total_products: 0, total_customers: 0,
})
const lowStock = ref([])
