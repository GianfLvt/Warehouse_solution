<template>
  <div>
    <h1 class="text-2xl font-bold mb-6">Nuovo Ordine Cliente</h1>
    <form @submit.prevent="save" class="card p-6 max-w-4xl space-y-6">
      ...
      <div>
        <div class="flex items-center justify-between mb-3">
          ...
          <div class="flex gap-2">
            <BarcodeScanner button-class="btn-secondary flex items-center gap-1 text-xs" @scanned="onScanOrderItem" />
        ...
        <div v-for="(item, i) in form.items" :key="i" class="flex flex-col sm:grid sm:grid-cols-12 gap-2 mb-3 p-3 sm:p-0 bg-gray-50 dark:bg-gray-700/30 sm:bg-transparent rounded-lg sm:items-end">
          ...
          <div class="sm:col-span-2 flex justify-end">
            <button type="button" @click="removeItem(i)" class="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded">
      ...
      <div class="flex gap-3">
        ...
        <router-link to="/orders" class="btn-secondary">Annulla</router-link>
      </div>
    </form>
  </div>
</template>
...
async function onScanOrderItem(code) {
  try {
